...
Build a redacted, deterministic release-readiness manifest for helix-workspace.
...
export async function buildReleaseReadinessManifest(options, dependencies = {}) {
  ...
  const dirtyRepositories = [workspace].filter((repository) => repository.dirty);
...
function validateFinalReleaseOptions(options) {
  ...
  for (const [label, value] of [["HELIX_RELEASE_WORKSPACE_TAG", options.workspaceReleaseTag]]) {
