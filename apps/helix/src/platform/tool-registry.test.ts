...
const actor: Actor = {
  ...
  orgId: "org-1",
  type: "agent",
  scopes: ["platform.read", "drive.write"],
};
...
describe("RuntimeToolRegistry", () => {
  ...
  it("does not record cost for failed tool executions", async () => {
    ...
    registry.register(
      tool({
        ...
        estimatedCostUsdMicros: usdToMicros(0.02),
        handler: async () => {
          throw new Error("backend unavailable: postgres://private:secret@database");
        },
      }),
    );
    registry.register(
  ...
  it("emits exactly one generic outcome for every invocation exit", async () => {
    ...
    await capture("pending", async (auditSink) => {
      const registry = createToolRegistry({
        auditSink,
        confirmationGate: new InMemoryConfirmationGate(new InMemoryPendingActionStore()),
      });
      registry.register(
        tool({
          id: "matrix.pending",
