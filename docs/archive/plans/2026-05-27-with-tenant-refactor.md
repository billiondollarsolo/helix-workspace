...
Every cross-tenant finding in the master review (`docs/archive/reviews/REVIEW.md`) has the same shape: a store method that takes an `orgId` parameter but forgets to use it (or accepts a request where `orgId` is missing and defaults to "any"). pgvector, the SMTP receiver, chat-permission subqueries, calendar RSVP, Drive share-target, app-password verify — same root cause. Adding a SOC2-grade audit requires demonstrating that tenant isolation is **structural**, not "we'll grep every PR." The compiler should refuse to compile a query that has no tenant.
...
export class SystemHandle {
  ...
  static unsafe(sql: postgres.Sql): SystemHandle {
    return new SystemHandle(sql);
  }
  raw(): postgres.Sql {
    return this.sql;
  }
...
import { TenantHandle, TenantQueryWithoutOrgIdError, withTenant } from "../src/db-tenant-scoped.js";
