...
export function renderChatBodyHtml(body: string, format: ChatBodyFormat): string {
  ...
  const fence = /^```([^\n`]*)\n([\s\S]*?)^```[ \t]*$/gmu;
  const fenced = [...safeBody.matchAll(fence)];
  if (fenced.length > 0) {
    let cursor = 0;
    const parts: string[] = [];
    for (const match of fenced) {
      parts.push(renderChatBodyHtml(safeBody.slice(cursor, match.index), format));
      const language = /^[a-z0-9+#._-]{1,32}$/iu.test(match[1] ?? "")
        ? (match[1] ?? "code").toLowerCase()
        : "code";
      parts.push(
        `<pre><code class="language-${escapeHtml(language)}">${escapeHtml((match[2] ?? "").replace(/\n$/u, ""))}</code></pre>`,
      );
      cursor = match.index + match[0].length;
    }
    parts.push(renderChatBodyHtml(safeBody.slice(cursor), format));
    return parts.join("");
  }
