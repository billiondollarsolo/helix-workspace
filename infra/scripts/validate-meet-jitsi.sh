...
assert_contains "$COMPOSE_FILE" 'HELIX_JITSI_PREPARE_UPLOAD: \$\{HELIX_JITSI_PREPARE_UPLOAD:-false\}' "Jibri must expose opt-in tenant storage prepare mode"
assert_contains "$COMPOSE_FILE" 'HELIX_JITSI_PREPARE_REQUIRED: \$\{HELIX_JITSI_PREPARE_REQUIRED:-false\}' "Jibri must expose prepare-required mode"
assert_contains "$COMPOSE_FILE" 'HELIX_JITSI_PREPARE_URL: \$\{HELIX_JITSI_PREPARE_URL:-\}' "Jibri must accept a recording upload prepare URL override"
