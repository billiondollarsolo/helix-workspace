      ...
      typeof input === "string"
        ...
        : input instanceof URL
          ...
          : input instanceof Request
            ? input.url
            : "";
    if (url.includes("/v1/api/admin/billing/account")) {
      return Promise.resolve(Response.json(apiBillingAccount));
    }
    if (url.includes("/v1/api/admin/billing/invoices")) {
      return Promise.resolve(Response.json(apiInvoices));
    }
    if (url.includes("/v1/api/admin/billing/usage")) {
...
function mockAdminBillingFailure(
  ...
  fetchMock.mockImplementation((input) => {
    ...
    if (url.includes("/v1/api/admin/billing/")) {
      ...
      typeof input === "string"
        ...
        : input instanceof URL
          ...
          : input instanceof Request
            ? input.url
            : "";
    if (url.includes("/v1/api/admin/security-policies")) {
...
describe("AdminConsole", () => {
        ...
        typeof input === "string"
          ...
          : input instanceof URL
            ...
            : input instanceof Request
              ? input.url
              : "";
      return url.includes("/v1/api/admin/users");
    });
  ...
  it("renders billing usage rollups from the billing API", async () => {
    ...
    await waitFor(() => {
      ...
      expect(requestedUrls).toContain(
        "/v1/api/admin/billing/usage?from=2026-05-01&to=2026-05-31&metricKey=storage_avg_bytes",
      );
    });
  });
  ...
  it("states a total billing outage once and retries every billing query", async () => {
    ...
    const retried = fetchMock.mock.calls
      ...
      .filter((url) => url.includes("/v1/api/admin/billing/"));
