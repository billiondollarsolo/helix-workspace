...
**Normative scope:** [Helix 1.0 scope](release/1.0-scope.md)
...
- **Shared file storage** with folders, versions, shares, downloads, and WebDAV
...
| Drive                | File storage only. **No native editing, file viewers, or converters.** Primary file action is **Download**. Desktop sync: open-source **rclone** over WebDAV — see `docs/drive-desktop-sync.md`. |
| Agents               | Non-read tool calls require human confirmation by default. Agents cannot approve their own actions.                                                                                              |
| Deployment           | Invite-only single-organization Business pilot (order of 5–50 trusted users). Public multi-tenant SaaS is deferred.                                                                              |
...
- Native document editing, file viewers, or converters
...
- Calendar and Meet are dormant behind the `full` profile and are outside 1.0.

Shipped surfaces are **Mail, Drive, Chat, Assistant, and Admin**. Public signup is outside the release. Only the workspace revision and its application/web image digests bind promotion.
