    ...
    ({ routePath, viewportName, themeName, rightRailState }) => {
      ...
      for (const element of overlapCandidates) {
        ...
        for (const [x, y] of points) {
          ...
          // A scroll container intentionally clips its off-screen descendants.
          // Only test hit points inside every clipping ancestor's content box.
          let clipped = false;
          for (let parent = element.parentElement; parent; parent = parent.parentElement) {
            const bounds = parent.getBoundingClientRect();
            const parentStyle = getComputedStyle(parent);
            if (
              (/auto|scroll|hidden|clip/u.test(parentStyle.overflowX) &&
                (x < bounds.left || x >= bounds.right)) ||
              (/auto|scroll|hidden|clip/u.test(parentStyle.overflowY) &&
