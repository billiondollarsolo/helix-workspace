...
This plan **consumes** the artifacts delivered by `docs/archive/plans/2026-07-18-cross-cutting-elite-standard.md`. Those must be merged first (or landed in the same branch train):
...
| **G8** | New mail capabilities (spam, filter.list, draft._, outbound.cancel, alias._) are tools first; web + CLI consume via contracts.                                                                               | A0, A2               |
...
describe("mailConfig", () => {
  ...
  it("defaults from-domain to localhost when unset", () => {
    expect(
      mailConfig(loadEnv({ DATABASE_URL: base.DATABASE_URL, REDIS_URL: base.REDIS_URL }))
        .fromDomain,
    ).toBe("localhost");
  });
});
...
- [ ] **Step 1: Write the failing test** — `stream.test.ts`: a fake `EventBus` publishes `activity.mail.received` for the subscribed actor's org; assert the SSE handler writes a `data: {"type":"mail.received",...}` frame; assert an event for a _different_ org is not delivered (authz filter).
- [ ] **Step 2: Run** → FAIL.
