...
/** Project the persisted scan lifecycle without exposing quarantined or unfinished bytes. */
export function driveUploadStateFromMetadata(
  status: unknown,
  deletedAt: Date | null,
): DriveUploadState {
  if (deletedAt !== null) return "trashed";
  switch (status) {
    case "prepared":
    case "pending_upload":
