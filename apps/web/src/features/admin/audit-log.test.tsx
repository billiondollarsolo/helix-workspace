...
describe("AuditLogList admin UI", () => {
  ...
  it("renders audit records with TanStack table semantics", async () => {
    ...
    expect(auditUrls()[0]).toBe("/v1/api/admin/audit-log?limit=50");
  ...
  it("applies filters and advances cursor pages through query params", async () => {
    ...
    await waitFor(() =>
      expect(auditUrls()[1]).toBe(
        "/v1/api/admin/audit-log?limit=50&objectType=credential&verb=agent.credential.created",
      ),
    );
    ...
    await waitFor(() =>
      expect(auditUrls()[2]).toBe(
        "/v1/api/admin/audit-log?limit=50&cursor=cursor-2&objectType=credential&verb=agent.credential.created",
      ),
    );
    expect(alertMock).not.toHaveBeenCalled();
    expect(confirmMock).not.toHaveBeenCalled();
  ...
  it("names the actor and keeps the raw id one level in for correlation", async () => {
    ...
    expect(directoryUrls()[0]).toBe("/v1/api/admin/users?includeDisabled=true&limit=250");
  });
  ...
  it("filters by actor server-side across the whole log, not just loaded rows", async () => {
    ...
    await waitFor(() =>
      expect(auditUrls()[1]).toBe(`/v1/api/admin/audit-log?limit=50&actorId=${ACTOR_ID}`),
    );
    expect(container.querySelector(".admin-page-subtitle")?.textContent).toContain(
  ...
  it("clears the actor filter on reset", async () => {
    ...
    await waitFor(() => expect(auditUrls().at(-1)).toBe("/v1/api/admin/audit-log?limit=50"));
  ...
  function mockApi(options: {
    ...
    fetchMock.mockImplementation((input) => {
      ...
      if (url.startsWith("/v1/api/admin/users")) {
  ...
  function auditUrls(): string[] {
    return requestUrls("/v1/api/admin/audit-log");
  }

  function directoryUrls(): string[] {
    return requestUrls("/v1/api/admin/users");
  }
...
describe("audit log API helpers", () => {
  it("builds list query params and validates response shape", async () => {
    ...
    expect(fetchMock.mock.calls[0]?.[0]).toBe(
      "/v1/api/admin/audit-log?limit=25&objectType=tool&verb=tool.invoked",
    );
