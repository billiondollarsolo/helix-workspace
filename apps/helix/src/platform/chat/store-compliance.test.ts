...
describe("Postgres Chat compliance invariants", () => {
  it("scopes retry dedupe by organization, room, actor, and client ID", async () => {
    ...
    expect(source).toContain("on conflict (org_id, actor_id, thread_id, client_message_id)");
    const retry = source.slice(
      source.indexOf("if (messageId === undefined)"),
      source.indexOf("if (messageId === undefined)") + 1300,
    );
    expect(retry).toContain("org_id = ${input.orgId}");
    expect(retry).toContain("thread_id = ${input.roomId}");
    expect(retry).toContain("actor_id = ${input.actorId}");
    expect(retry).toContain("client_message_id = ${input.clientMessageId ?? null}");
  });
