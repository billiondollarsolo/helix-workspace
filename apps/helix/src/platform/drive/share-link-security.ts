...
export async function verifyDriveSharePassword(
  ...
  const [algorithm, saltText, digestText] = parts;
  if (
    parts.length !== 3 ||
    algorithm !== "scrypt" ||
    saltText === undefined ||
  ...
  try {
    ...
    if (salt.byteLength !== 16 || expected.byteLength !== passwordKeyBytes) return false;
    const actual = (await deriveScrypt(password, salt, passwordKeyBytes)) as Buffer;
