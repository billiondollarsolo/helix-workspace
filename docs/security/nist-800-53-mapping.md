...
800-53 Rev. 5 control families. It is an _implementation_ mapping intended to
...
- The FIPS 140-3 _module validation_ itself — Helix consumes a validated
...
- Cluster-level STIG enforcement (admission control via Kyverno/Gatekeeper) —
  Helix supplies the policy _contract_ in `values-sovereign.yaml` and
