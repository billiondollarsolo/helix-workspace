...
**What's wrong**: The insert is `on conflict do nothing`. So sharing a file to actor X as `editor` when X already has it as `reader` is a silent no-op — the higher role is _not_ granted. Same problem when re-sharing with a longer expiry: the old expiry sticks. There's no `drive.unshare` tool at all (the only way to revoke is to call permission-store APIs directly), and no way to _change_ a role.
...
**What's wrong**: `targetActorIds` is taken verbatim from `drive.share` input and inserted into `permissions` with the caller's `orgId`. The schema only requires UUIDs. A caller can grant access to _any_ UUID — including the actor id of a user from another tenant — and the permissions row will be created. Whether the cross-tenant actor can then actually read depends on whether the read path also enforces `org_id` on the actor lookup (it does, but the _grant being possible_ is the bug — it leaks structural information and creates phantom grants that future code might trust).
...
**Fix**: Compute the storage-key set in the tx, commit, _then_ fire deletes in parallel (or via an outbox-driven background job that's idempotent on missing keys).
**Effort**: S
...
### S27: Storage-key validation rejects only the _finalize_ override, not the _prepare_ path · severity: LOW · category: security
