...
export const V2_NEGATIVE_SECURITY_MATRIX = [
  ...
  boundary("Drive", "object ID guessing; quarantined file; revoked share/link; wrong KMS policy", [
    ...
    testCase(
      ...
      "Unknown, revoked, expired, and password-protected public links return the same non-enumerable credential challenge.",
      ref(
        "apps/helix/src/platform/drive/routes.test.ts",
        "uses one credential challenge for unknown, revoked, expired, and protected tokens",
      ),
    ),
    testCase(
  ...
  boundary("Chat", "non-member list/search/subscribe/send; wrong-origin valid cookie", [
    ...
    testCase(
      ...
      ref(
        ...
        "rejects an inaccessible ticket room before accepting frames",
      ),
    ),
    testCase(
      ...
      ref(
        "apps/helix/src/platform/chat/realtime.test.ts",
        "rejects unsafe browser origins before consuming a ticket",
      ),
    ),
  ]),
  boundary(
  ...
  boundary("Webhook", "invalid signature, replay, wrong tenant, duplicate", [
    testCase(
      ...
      ref(
        ...
        "rejects missing, stale, and tampered signatures before mutation",
      ),
    ),
    testCase(
      "webhook.replay",
      ...
      ref(
        "apps/helix/src/platform/mail/delivery-events.integration.test.ts",
        "isolates idempotency by actor and preserves bounce feedback ahead of dispatch acknowledgement",
        "live-postgres",
      ),
    ),
    testCase(
