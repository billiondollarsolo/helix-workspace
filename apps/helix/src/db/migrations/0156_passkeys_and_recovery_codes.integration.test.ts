...
describe.skipIf(process.env.DATABASE_URL === undefined)(
  "Postgres passkey and recovery invariants",
  () => {
    ...
    it("rejects duplicate credentials and a cloned single-device counter", async () => {
      ...
      await expect(
        sql`update passkey set counter = 2 where id = 'iam12-key'`,
    ...
    it("consumes password reset tokens once and revokes every active session", async () => {
      ...
      try {
        ...
        const requested = await runtime.auth.handler(
          new Request("https://auth.helix.test/api/auth/request-password-reset", {
            method: "POST",
            headers: { "content-type": "application/json", origin: "https://auth.helix.test" },
            body: JSON.stringify({
              email: "iam12@example.test",
              redirectTo: "https://auth.helix.test/login",
            }),
          }),
        );
        ...
        const reset = () =>
          runtime.auth.handler(
            new Request("https://auth.helix.test/api/auth/reset-password", {
              method: "POST",
              headers: { "content-type": "application/json", origin: "https://auth.helix.test" },
    ...
    it("atomically revokes every factor and session during an administrator reset", async () => {
      ...
      await expect(
        new PostgresAdminUsersStore(sql).resetMfa({
          orgId,
    ...
    `;
      expect(evidence).toEqual([
        expect.objectContaining({
          verb: "identity.mfa.reset",
          this_hash: expect.stringMatching(/^[a-f0-9]{64}$/u),
        }),
      ]);
    });
  },
