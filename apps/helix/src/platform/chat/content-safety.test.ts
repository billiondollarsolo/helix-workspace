...
it("escapes fenced code while retaining Markdown around it", () => {
  const html = renderChatBodyHtml(
...
it("renders attachment-only messages and tombstones without a body", () => {
  expect(renderChatBodyHtml("", "plain")).toBe("");
  expect(renderChatBodyHtml("```js\nconst x = 1;\n```", "markdown")).toBe(
