...
export const SECURITY_POLICY_RUNTIME_CAPABILITIES: readonly PolicyRuntimeCapability[] = [
  ...
  },
  {
    ...
    mode: "recorded_only",
    summary:
      "Private-key OIDC login is connected. Requiring SSO for every login and disabling local recovery are not enforced by this policy.",
    enforcementPoints: [],
  },
  {
    policyType: "session",
    mode: "enforced",
    summary:
      "Tenant sessions enforce absolute and idle lifetime, concurrent-session limits, and administrative reauthentication.",
    enforcementPoints: [
      "PostgresBetterAuthSessionPolicyAuthorizer",
      "helix_authorize_tenant_session",
    ],
  },
  {
  ...
  },
  {
    ...
    mode: "enforced",
    summary:
      "Tenant DLP checks content and resource classification on mail, chat, Drive sharing and downloads, exports, and agent tool calls.",
    enforcementPoints: [
      "TenantDlpGuard",
      "tool registry",
      "mail dispatch",
      "Drive and Chat routes",
    ],
  },
  {
  ...
  {
    policyType: "drive_workflows",
    mode: "enforced",
    summary: "Drive workflow policy governs review assignment, approval, and sensitivity changes.",
    enforcementPoints: ["PostgresDriveWorkflowStore"],
  },
] as const;
