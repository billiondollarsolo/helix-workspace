...
- `docs/archive/reviews/mail.md` — 25 findings (1 CRITICAL, 8 HIGH)
- `docs/archive/reviews/drive.md` — 28 findings (4 HIGH, including SSRF preview + OOM streaming)
- `docs/reviews/docs.md` — 22 findings + dedicated OnlyOffice removal punch list
- `docs/archive/reviews/meet.md` — 18 findings (post-wave-1 fixes — JWT rotation, webhook auth, mock-recorder gating)
- `docs/archive/reviews/chat-calendar.md` — 39 findings (chat 16 + calendar 19 + carddav 4)
- `docs/archive/reviews/ai-assistant.md` — 27 findings (pgvector tenant leak, classification trust boundary)
- `docs/archive/reviews/admin-platform.md` — 34 findings (SCIM, OAuth, app-passwords, MFA, plugins)
- `docs/archive/reviews/REVIEW.md` — master backlog: **197 items** (6 CRITICAL, 50 HIGH, 76 MEDIUM, 65 LOW), prioritized fix waves, cross-cutting themes
...
5. **Slides concurrent-edit data loss** — Option B (interim safety net) shipped: per-slide `expectedVersion` CAS, 409 + WS rebroadcast on conflict, migration 0062. Full per-shape OT tracked in `docs/archive/reviews/follow-up.md`.
...
- Build the proper per-shape Slides OT (Wave-1 shipped CAS interim — `docs/archive/reviews/follow-up.md`)
