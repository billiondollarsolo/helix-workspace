...
describe("release-readiness manifest", () => {
  ...
  it("creates a deterministic manifest for a clean workspace repository", async () => {
  ...
  it("requires a valid immutable image digest", async () => {
    const fixture = await createFixture();
    const baseArgs = ["--workspace-dir", fixture.workspace, "--evidence-dir", fixture.evidence];
    await expect(
  ...
  it("requires passed Chat C6 evidence at the full release pilot-load profile", async () => {
    ...
    tooBrief.profile.durationSeconds = 1799;
    tooBrief.scenarios.pilot_load.evidence.durationSeconds = 1799;
  ...
  it("requires genuine passed V4 failure/recovery evidence and publishes safe metrics", async () => {
    ...
    await writeFile(
      evidencePath,
      `${JSON.stringify(createStaticFailureRecoveryEvidence(new Date("2026-07-28T20:00:00.000Z")))}\n`,
      "utf8",
    );
    const args = liveEvidenceArgs(
      fixture,
  ...
  it("fails final-release mode on missing gates, binding mismatches, and external Mail gaps", async () => {
    ...
    await writeFinalReleaseEvidence(fixture, binding);
    const unknownBinding = JSON.parse(await readFile(drivePath, "utf8"));
    unknownBinding.releaseBinding.unknownField = "f".repeat(40);
    await writeFile(drivePath, `${JSON.stringify(unknownBinding)}\n`, "utf8");
    await expect(buildFinalManifest(fixture, args)).rejects.toThrow(
  ...
  it("fails final promotion on incomplete, stale, unsafe, or unsigned R0-R3 artifacts", async () => {
    ...
    const cases = [
      ...
      },
      {
        path: "full-gates-evidence.json",
        mutate: (report) => {
          report.workspace.revision = "f".repeat(40);
        },
        error: "workspace gate revision does not match",
      },
      {
        path: "business-readiness-evidence.json",
        mutate: (report) => {
          report.limits.directMx = true;
        },
  ...
  it("verifies offline image provenance against the trusted issuer and paired revisions", async () => {
    ...
    await mutateImageProvenance(fixture, "application", (document) => {
      ...
      payload.predicate.workspace.sha = "9".repeat(40);
      resignProvenancePayload(fixture, document, payload);
    });
    await expect(buildFinalManifest(fixture, args)).rejects.toThrow(
      "does not bind the exact workspace revision",
    );
    await writeFinalSupportingEvidence(fixture, binding);
    await mutateImageProvenance(fixture, "application", (document) => {
  ...
  const reports = {
    ...
    "slo-soak-evidence.json": {
      ...
      profile: {
        ...
        driveMaximumObjectBytes: 1073741824,
      ...
      objectives: {
        ...
        chatVisibleP95Ms: 1500,
        mailAcceptanceP95Ms: 50000,
      },
...
function createSigstoreProvenanceArtifact(fixture, binding, subjectName, subjectDigest) {
  const payload = Buffer.from(
    JSON.stringify({
      ...
      predicateType: "https://helix.billiondollarsolo.com/attestations/source-provenance/v1",
      predicate: {
        schemaVersion: 1,
        workspace: {
...
function createTestRekorEntry(fixture, envelope, payload, options = {}) {
  ...
  const integratedTime = Math.floor(
    Date.parse(options.integratedAt ?? "2026-07-29T21:00:00.000Z") / 1000,
  );
...
function passedChatEvidence() {
  ...
  evidence.scenarios = {
    ...
    pilot_load: passedChatScenario(timestamp, {
      ...
      actualSockets: 100,
      durationSeconds: 1800,
      messagesAttempted: 1800,
      messagesObserved: 1800,
      errors: 0,
      errorRate: 0,
      p95LatencyMs: 100,
      p99LatencyMs: 200,
...
`,
  ...
  // Evidence timestamps are fixed; certificate validity must not depend on the test-run date.
  const caConfigPath = resolve(root, "fulcio-ca.cnf");
  await writeFile(resolve(root, "fulcio-index"), "");
  await writeFile(resolve(root, "fulcio-serial"), "01\n");
  await writeFile(
    caConfigPath,
    `[ca]
default_ca = issuer
[issuer]
database = ${resolve(root, "fulcio-index")}
serial = ${resolve(root, "fulcio-serial")}
new_certs_dir = ${root}
certificate = ${caCertificatePath}
