...
function assertRejected(
  ...
  if (caught instanceof ProductionConfigurationError) {
    expect(caught.issues).toEqual(
      expect.arrayContaining([expect.objectContaining({ variable: expectedVariable })]),
    );
  } else {
    expect((caught as Error).message).toContain(`  - ${expectedVariable}:`);
  }
...
describe("assertProductionConfiguration", () => {
  ...
  it.each([
    ...
    ["full allowlist under mvp", "mail,drive,chat,assistant,calendar,meet"],
  ...
  it.each(["calendar", "meet"] as const)(
  ...
  it("accepts Full Workspace profile when deps are present (boot path)", () => {
    ...
    const env = loadEnv({
      ...productionFixture(),
      HELIX_WORKSPACE_PROFILE: "full",
      HELIX_APPS: "mail,drive,chat,assistant,calendar,meet",
