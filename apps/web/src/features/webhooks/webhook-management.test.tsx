...
describe("WebhookManagement", () => {
  ...
  beforeEach(() => {
    ...
    fetchMock = vi.fn<typeof fetch>((input) => {
      ...
      if (input === "/v1/api/tools/webhook.inbound.list") {
      ...
      if (input === "/v1/api/tools/webhook.overview") {
  ...
  it("renders outbound, inbound, and virtualized delivery tables from query data", async () => {
    ...
    expect(fetchBody("/v1/api/tools/webhook.overview")).toMatchObject({ deliveryLimit: 100 });
    expect(toolCallUrls()).toEqual(["/v1/api/tools/webhook.overview"]);
  ...
  it("passes webhook id and time range delivery filters through to the list API", async () => {
    ...
    await waitFor(() =>
      expect(latestFetchBody("/v1/api/tools/webhook.delivery.list")).toMatchObject({
        direction: "outbound",
        status: "failed",
  ...
  it("submits outbound form values as a create payload", async () => {
    ...
    await waitFor(() =>
      expect(fetchBody("/v1/api/tools/webhook.outbound.create")).toMatchObject({
  ...
  it("keeps invalid outbound create payloads local to the editor", async () => {
    ...
    expect(
      fetchMock.mock.calls.some((call) => call[0] === "/v1/api/tools/webhook.outbound.create"),
    ).toBe(false);
  });
  ...
  it("keeps invalid inbound create payloads local to the editor", async () => {
    ...
    expect(
      fetchMock.mock.calls.some((call) => call[0] === "/v1/api/tools/webhook.inbound.create"),
    ).toBe(false);
  });

  it("optimistically creates outbound webhooks and rolls back failed saves", async () => {
    const createResponse = defer<Response>();
    deferredResponses.set("/v1/api/tools/webhook.outbound.create", createResponse);
    renderWebhooks();
    await waitForText("Slack launch channel");
  ...
  it("optimistically creates inbound webhooks and rolls back failed saves", async () => {
    const createResponse = defer<Response>();
    deferredResponses.set("/v1/api/tools/webhook.inbound.create", createResponse);
    renderWebhooks();
    await waitForText("Slack launch channel");
  ...
  it("optimistically rotates inbound secrets and rolls back failed refreshes", async () => {
    ...
    deferredResponses.set("/v1/api/tools/webhook.inbound.rotate-secret", rotateResponse);
    renderWebhooks();
    await waitForText("Slack launch channel");
  ...
  it("optimistically applies outbound row actions and rolls back failed actions", async () => {
    ...
    deferredResponses.set("/v1/api/tools/webhook.outbound.update", updateResponse);
    renderWebhooks();
    await waitForText("Slack launch channel");
  ...
  it("optimistically applies inbound row actions and rolls back failed actions", async () => {
    const updateResponse = defer<Response>();
    deferredResponses.set("/v1/api/tools/webhook.inbound.update", updateResponse);
    renderWebhooks();
    await waitForText("Slack launch channel");
    ...
    return [
      ...new Set(
        fetchMock.mock.calls
          ...
          .filter((url) => url.startsWith("/v1/api/tools/")),
      ),
    ];
  }
