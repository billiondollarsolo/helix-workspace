...
describe("AppPasswordsManagement", () => {
  ...
  it("lists active app passwords and can include revoked app passwords", async () => {
    fetchMock.mockImplementation((input, init) => {
      ...
      if (input === "/v1/api/tools/app.passwords.list") {
  ...
  it("creates an app password through the pending tool approval flow and shows the one-time password", async () => {
    fetchMock.mockImplementation((input) => {
      ...
      if (input === "/v1/api/tools/app.passwords.create") {
        return Promise.resolve(
          Response.json(
    ...
    expect(fetchBody("/v1/api/tools/app.passwords.create")).toMatchObject({
  ...
  it("revokes an app password through the shared confirmation dialog without native confirm", async () => {
    fetchMock.mockImplementation((input) => {
      ...
      if (input === "/v1/api/tools/app.passwords.revoke") {
        return Promise.resolve(
          Response.json(
    ...
    await waitFor(() => {
      expect(fetchBody("/v1/api/tools/app.passwords.revoke")).toEqual({
  ...
  it("cancelling the revoke confirmation sends nothing", async () => {
    ...
    expect(
      fetchMock.mock.calls.some((call) => call[0] === "/v1/api/tools/app.passwords.revoke"),
    ).toBe(false);
    ...
    act(() => {
      root.render(
        createElement(
          QueryClientProvider,
          { client: queryClient },
          withAdminRouter(createElement(AppPasswordsManagement)),
        ),
      );
    });
  }
  ...
  function latestListBody() {
    const listCalls = fetchMock.mock.calls.filter(
      (call) => call[0] === "/v1/api/tools/app.passwords.list",
    );
