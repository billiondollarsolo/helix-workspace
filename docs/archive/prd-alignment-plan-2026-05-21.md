...
**Date:** 2026-05-21
**Method:** Five parallel read-only audits cross-referencing docs/archive/PRD.md (3,102 lines, ~90 roadmap
...
- `apps/helix/src/server.ts` **never loads the `plugins/` directory as code**. It only
  _discovers manifests_ (`platform/plugins/tools.ts`) to populate the admin catalog UI.
...
Until this is decided, treat everything below as work on the _monolith as it stands_.
...
The tier engine _declares_ defaults but no code enforces: internal mTLS (only an example
...
are Helm placeholders. NIST 800-53 doc is a 36-line mapping. _Correctly scoped as scaffolding per
TASK-A02 and open-question #9 — track as a deliberate post-v1 item, not a silent gap._
...
1. **Plugin architecture (§2): ✅ RESOLVED — confirmed hybrid model.** The headline finding
   ...
   - **Core apps** (mail, chat, drive, docs, calendar, meet, assistant) are **toggleable
     modules of the Helix platform** — _not_ plugins and _not_ per-user containers. They ship
...
_Generated from a five-agent parallel audit on 2026-05-21. Every claim is traceable to file
evidence in the audit transcripts; `docs/archive/prd-validation-gap-matrix.md` remains the
running task-level status map and should be reconciled against this plan._
