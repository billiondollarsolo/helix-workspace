...
describe("production-readiness source workflow", () => {
  ...
  it("executes the mandatory V2 tenant boundary against migrated live PostgreSQL", () => {
    const migration = liveDemoJob.indexOf("name: Run database migrations");
    ...
    expect(liveDemoJob).toContain('HELIX_REQUIRE_LIVE_SUITES: "1"');
    expect(liveDemoJob).not.toMatch(/cross-tenant-isolation\.test\.ts\s+-t\s/u);
