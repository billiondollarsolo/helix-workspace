...
function countVerification(domains: readonly DomainWithRecords[]): {
  ...
  return {
    verified: domains.filter((it) => it.domain.status === "verified").length,
    pending: domains.filter((it) => it.domain.status === "pending").length,
    failed: domains.filter((it) => it.domain.status === "quarantined").length,
  };
}
  ...
  return (
    <dl className="m-0 grid gap-1 text-xs">
      ...
      {requirementLines.length === 0 ? null : (
        <div>
          <dt className="sr-only">Attention</dt>
          <dd className="m-0 mt-1 text-[var(--text-2)]">
            Attention: {nameList(requirementLines, 4)}.
          </dd>
        </div>
      )}
    </dl>
  ...
  return (
    <dl className="m-0 grid gap-1 text-xs">
      ...
      {domainNames.length === 0 ? null : (
        <div>
          <dt className="sr-only">Registered domains</dt>
          <dd className="m-0 mt-1 text-[var(--text-2)]">Registered: {nameList(domainNames, 4)}.</dd>
        </div>
      )}
    </dl>
  );
