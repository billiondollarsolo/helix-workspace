...
describe("Jibri recorder health", () => {
  it("reports available only for a healthy idle recorder", async () => {
    ...
    await expect(
      createJibriRecorderHealthCheck("http://jibri.internal:2222/jibri/api/v1.0/health", fetchImpl)(),
    ).resolves.toBe(true);
  });
  ...
  it.each([
    ...
    await expect(createJibriRecorderHealthCheck("https://jibri.example/health", fetchImpl)()).resolves.toBe(
      false,
    );
  });
  ...
  it("fails closed on transport and HTTP errors", async () => {
    ...
    await expect(createJibriRecorderHealthCheck("https://jibri.example/health", rejected)()).resolves.toBe(
      false,
    );
    await expect(
