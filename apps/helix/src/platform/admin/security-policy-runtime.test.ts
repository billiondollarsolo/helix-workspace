...
describe("SECURITY_POLICY_RUNTIME_CAPABILITIES inventory (ADM.1)", () => {
  ...
  it("marks sharing and DLP as enforced, SSO and device trust as recorded only", () => {
...
describe("SSO / recorded-only required validation (ADM.4/ADM.5/ID.4)", () => {
  ...
  it("allows required DLP and refuses required device trust", () => {
    expect(validateRecordedOnlyRequiredEnforcement("dlp", "required").ok).toBe(true);
