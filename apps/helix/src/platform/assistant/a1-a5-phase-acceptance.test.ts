...
const human: Actor = {
  ...
  orgId,
  type: "user",
  scopes: ["drive.read", "drive.write", "admin.config.read"],
};
const agent: Actor = {
  id: "agent-1",
  orgId,
  ...
  it.each([
    ...
    expect(
      evaluateToolPolicyFirewall({
        actor,
        tenantId: orgId,
        tool: { id: `demo.${sideEffects}`, permission: "drive.write", sideEffects },
  ...
  it("fails closed for unknown tools/effects/channels and tenant mismatch", () => {
    ...
    expect(
      evaluateToolPolicyFirewall({
        actor: agent,
        tenantId: "other",
        tool: { id: "demo.write", permission: "drive.write", sideEffects: "write" },
        effectiveClassification: "standard",
        sourceProvenance: { sourceIds: [], containsUntrustedContext: false },
        requestChannel: "mcp",
        tier: "business",
