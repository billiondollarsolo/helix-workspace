...
async function startReceiver(
  ...
  const receiver = new SmtpMailReceiver({
    ...
    resolveRecipient: async (address) => {
      const resolved = await resolveRecipient(address);
      return resolved === null
        ? null
        : { orgId: resolved.orgId, actorId: resolved.actorId, address: resolved.normalizedAddress };
    },
    authenticator: {
      async authenticate() {
...
class DurableRecordingStore {
  ...
  async insertInboundMessage(input: MailMessageInput): Promise<StoredMailMessage> {
    ...
    if (existing !== undefined) {
      return { ...existing, created: false, deliveredActorIds: [] };
    }
