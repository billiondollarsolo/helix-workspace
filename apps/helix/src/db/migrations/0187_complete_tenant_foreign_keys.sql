-- Reapply the catalog upgrade from 0111 to relationships introduced afterward.
-- Keep that shipped migration immutable; preserve FK actions and fail on dirty data.
do $$
declare
...
begin
  for actor_reference in
    ...
    where namespace.nspname = 'public'
      ...
      -- Deletion proofs allocate a detached identity before the purge creates
      -- its audit principal, and must remain available after tenant removal.
      and (relation.relname, column_row.attname) <> ('tenant_deletion_proofs', 'system_actor_id')
      and (
